...
turbine_categories = [
        dict(turbine=get_oedb_windturbineconfig(134), up=1900.),
        dict(turbine=get_oedb_windturbineconfig(73), up=2200.),
        dict(turbine=get_oedb_windturbineconfig(119), up=2500.),
        dict(turbine=get_oedb_windturbineconfig(51), up=50000.)
    ]
...
for turbine_cat in turbine_categories:
    ...
    wind[name] = cutout.wind(
        turbine=turbine_cat['turbine'], layout=turbine_cat['layout'],
        show_progress=False)
...
def compare_power_generation(
        renewable_technology, generation1, generation2, resample_freq="1d"):
...
wind_model = wind['total'].squeeze().to_series()
wind_model.name = 'Atlite'

compare_power_generation(
    "Wind Onshore", wind_model,
    opsd['DE_wind_generation_actual'], resample_freq="1H")


compare = pd.DataFrame(
    dict(atlite=wind.squeeze().to_series(), opsd=opsd['DE_wind_generation_actual'] /1e3)) # in GW
